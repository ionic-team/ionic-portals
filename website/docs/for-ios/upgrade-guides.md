...
`Portal.sync()` continues to work unchanged for `.ionic` sources. The new `.provider` case allows a Portal to instead sync from any external live update service. See [Using a Live Update Provider](./live-update-provider.md).
